...
<body>
    ...
    <main>
        ...
        <div class="form">
            ...
            <div>
                <a href="add_client.php"><button class="button">Załóż konto użytkownika</button></a>
            </div>
        </div>
        <?php
            if(@$_SESSION['good']==1) echo "<script>alert('Nieprawidłowy login lub hasło!');</script>";
            unset($_SESSION['good']);
        ?>
        <?php else : include('get_data.php')?>
        <p id="welcome">Witaj, <?=$imie." ".$nazwisko?></p>
        <div id="wrapp">
            <div class="button link">
                <a href="show_services.php">Przejrzyj swoje zlecenia</a>
            </div>
            <div class="button link">
                <a href="add_service.php">Dodaj nowe zlecenie</a>
            </div>
        </div>
        <?php endif; ?>
